...
public class StockOperationCreatedHandler(
    IPortfolioRepository repository,
    ILogger<StockOperationCreatedHandler> logger) : INotificationHandler<StockOperationCreatedEvent>
{
    private readonly ILogger _logger = logger;
    private readonly IPortfolioRepository repository = repository;

    public async Task Handle(StockOperationCreatedEvent domainEvent, CancellationToken cancellationToken)
    {
        _logger.LogInformation("StockOperationCreatedEvent - {OperationId}", domainEvent.Operation.Id);
